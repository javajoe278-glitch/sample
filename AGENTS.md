...
- **Centralized config**: `config/defaults.json` is the single source of truth for version pins (agent-server, automation, automation SDK), port defaults, persistence paths, package names, and transitive Python constraints (`constraints.agentClientProtocol`, `constraints.mcp`). All consumers read from this file:
