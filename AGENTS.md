...
- The launcher → backend → frontend → suffix plumbing is:
  ...
  - `scripts/ingress.mjs` and `scripts/static-server.mjs` proxy the real agent-server `/server_info` response and append `runtime_services` when configured. A direct Vite proxy does the same when its launcher supplies `VITE_RUNTIME_SERVICES_INFO`; all three share the response helper in `scripts/proxy-utils.mjs`, including compressed JSON handling and unchanged fallback for errors or invalid metadata. This keeps version/tool compatibility fields authoritative from the SDK while letting the Agent Canvas stack advertise automation/frontend/ingress topology.
