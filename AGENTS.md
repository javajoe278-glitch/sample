...
- Auth modes for `agent-canvas` (dev and production):
  ...
  - **Key rotation resilience (non-public):** `syncLauncherLocalBackends()` in `src/api/backend-registry/storage.ts` re-runs at module init. It overwrites `apiKey` (and bumps `connectionRevision`) from the current `makeDefaultLocalBackend().apiKey` (sourced from `VITE_SESSION_API_KEY` or, in the published-binary path, `window.__AGENT_CANVAS_SESSION_API_KEY__`) for every stored **local** backend whose host is the served origin (`isServedOriginHost`, port-sensitive), plus the seeded `"default-local"` entry when its host is loopback-equivalent to the launcher default. Custom backends on a different host or loopback port keep their stored key. E2E coverage: `tests/e2e/mock-llm/backends/mock-llm-auth-modes.spec.ts` (fresh-install, key-rotation, and public-mode scenarios).
