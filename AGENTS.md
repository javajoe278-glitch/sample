...
- **Dual-stack binding**: `scripts/static-server.mjs` and `scripts/ingress.mjs` default to `127.0.0.1` (loopback) so a local session key is not exposed on the LAN. The Docker entrypoint passes `--host ::` (dual-stack, all interfaces inside the container) so published ports work, but only passes `--allow-lan-session-key` when `AGENT_CANVAS_ALLOW_LAN_SESSION_KEY=true`; the documented opt-in pairs that setting with a host-loopback-only port publish. Playwright npm configs should use `http://127.0.0.1` for the canvas origin when talking to the default bind. The mock LLM server URL (`MOCK_LLM_URL`) still uses `127.0.0.1` because the Python mock server is a separate process whose bind behavior we don't control.
