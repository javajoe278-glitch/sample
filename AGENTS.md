...
- The launchers (`agent-canvas`, `dev`, `dev:static`, `dev:minimal`) accept a repeatable `--skills <source>` (or `OH_SKILLS_SOURCES`) for external skills sources — local directories, git URLs, or `host/path` shorthand. `scripts/skills-sources.mjs` resolves them best-effort at startup into a manifest under the state dir; static-server injects it via `--external-skills-file` and the Vite dev server via `VITE_EXTERNAL_SKILLS_FILE`, both as the `window.__AGENT_CANVAS_EXTERNAL_SKILLS__` JSON string. `src/utils/external-skills.ts` reads that global; external skills list in Customize → Skills under the "external" scope, are allow-listed under source-qualified `enabled_skills` keys (`external:<source-id>:<name>`, off by default), and enabled ones join `agent_context.skills` in `buildAgentContext`.
