...
describe("dev-static", () => {
  ...
  it("advertises the resolved frontend port in CORS origins", () => {
    const env = buildAutomationBackendEnv(
      {
        agentServerPort: 18000,
        ingressPort: 8000,
    ...
    // hardcoding 3001, so an auto-fallback or --frontend-port override keeps
    // the browser origin authorized when the frontend is browsed directly.
